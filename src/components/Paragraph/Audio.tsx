...
const Audio = (data:any) => {
  ...
  useEffect(() => {
    const getTranscriptText = async function (node_id:number) {
      const response = await fetch(`${process.env.NEXT_PUBLIC_DRUPAL_BASE_URL}/${locale}/api/v1/transcript/${node_id}`)
  ...
  return (
    <>
      <audio className="mb-8 w-full" controls>
        <source src={absoluteUrl(data?.field_audio?.uri?.url)} type={data?.field_audio?.filemime || 'audio/mp3'} />
      </audio>
      {
        transcriptText.length > 0 && (
          <>
            <button className="show-transcript btn-primary mb-2 flex justify-self-end" onClick={() => setShowTranscript(!showTranscript)}> {showTranscript ? 'Hide' : 'Show'} Transcript </button>
            { showTranscript &&
